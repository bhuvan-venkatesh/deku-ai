...
Mat Image_Classifier::get_descriptors(const Mat& image,
...
vector< DMatch > Image_Classifier::matches_within_tolerance(const vector< DMatch >& matches, double tolerance){
...
void Image_Classifier::detect_mario(const Mat& img_scene, vector<int32_t>& ret,
  ...
  for(size_t i = 0; i != templates.size(); ++i){
    ...
    obj_corners[0] = cvPoint(0,0); obj_corners[1] = cvPoint( cols, 0 );
    obj_corners[2] = cvPoint( cols, rows ); obj_corners[3] = cvPoint( 0, rows );
    ...
    if(abs(prev_y-mario.y) < mario_tolerance && abs(prev_x-mario.x) < mario_tolerance ||
      prev_y == -1 && prev_x == -1){
        prev_y=mario.y;
        prev_x=mario.y;
      for(int32_t cell = (int)(mario.x/block_width*side+mario.y/block_height)
